USE [DBATOOLS]
GO
/****** Object:  StoredProcedure [dbo].[sp_XMLtoData]    Script Date: 29/05/2022 17:56:22 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE OR ALTER PROCEDURE [dbo].[sp_XMLtoData]	
...
BEGIN
	...
	*************************************************************************/
	BEGIN TRY
		SET @v_RunCommand = 'EXEC ' + QUOTENAME(@v_DatabaseName) + '.' + QUOTENAME(@v_SchemaName) + '.' + QUOTENAME(@v_ObjectName)
				...
				+ ', @p_ID = ' + COALESCE(CAST(@p_ID AS VARCHAR(MAX)), 'NULL');
	...
	*************************************************************************/
	BEGIN TRY
		...
		-- Inserting XML into temp table
		SELECT @p_XML AS response
		INTO #tmp

		-- Store inserts requests to temp table
		SELECT A.id,
			A.parentid,
			'INSERT INTO ' + QUOTENAME(@p_Database) + '.' + QUOTENAME(@p_Schema) + '.' + QUOTENAME([dbo].[fn_TableName_from_Node](@p_TablePrefix, A.localname)) + ' 
		(ID_' + COALESCE([dbo].[fn_TableName_from_Node](@p_TablePrefix, C.localname), '') + COALESCE(', ' + STRING_AGG(B.name, ', '), '') + ')
		SELECT ' + CASE WHEN A.parentid IS NULL THEN CONVERT(VARCHAR, @p_ID) ELSE '%ID%' END + COALESCE(', 
		' +  STRING_AGG('response.value(''(' + B.localname + '/node())[' + CONVERT(VARCHAR, A.Nb) + ']'', ''nvarchar(max)'')', ', ' + CHAR(10)), '') + ' 
		FROM #tmp' AS request,
		'SELECT MAX(ID) AS maxID FROM ' + QUOTENAME(@p_Database) + '.' + QUOTENAME(@p_Schema) + '.' + QUOTENAME([dbo].[fn_TableName_from_Node](@p_TablePrefix, A.localname)) AS maxRequest,
			NULL AS insertedID
		INTO #final
		FROM [DBATOOLS].[dbo].[fn_list_xml_nodes] (@v_docHandle) A
			INNER JOIN [DBATOOLS].[dbo].[fn_list_xml_nodes] (@v_docHandle) D ON D.parentid = A.id AND D.name IS NOT NULL
			LEFT JOIN [DBATOOLS].[dbo].[fn_list_xml_nodes] (@v_docHandle) B ON B.parentid = A.id AND B.isColumn = 1 AND B.id = D.id
			LEFT JOIN [DBATOOLS].[dbo].[fn_list_xml_nodes] (@v_docHandle) C ON A.parentid = C.id
		GROUP BY A.id, A.parentid, A.localname, A.Nb, C.localname;
		
		DECLARE @v_SQL NVARCHAR(MAX);
		DECLARE @v_SQLMax NVARCHAR(MAX);
		DECLARE @v_ID BIGINT;
		...
		OPEN table_cursor;
		FETCH NEXT FROM table_cursor INTO @v_ID, @v_ParentID, @v_SQL, @v_SQLMax;

		WHILE @@FETCH_STATUS = 0  
		BEGIN  
			-- Get the insertedID from parent for the FK
			SELECT @p_ID = insertedID FROM #final WHERE id = @v_ParentID;

			-- Put the ID into the request
			SET @v_SQL = REPLACE(@v_SQL, '%ID%', CONVERT(VARCHAR, @p_ID));

			-- Insert the line
			EXEC sp_executesql @v_SQL;

			-- Retrieve ID from inserted line
			DROP TABLE IF EXISTS #maxID;
			CREATE TABLE #maxID (maxID bigint);
			INSERT INTO #maxID
			EXEC sp_executeSQL @v_SQLMax;
			SELECT @p_ID = maxID FROM #maxID;

			-- Store the ID of inserted line to temp table
			UPDATE #final SET insertedID = @p_ID WHERE id = @v_ID;
